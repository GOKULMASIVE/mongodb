...
app.delete(`/movies/:id`, async function (request, response) {
  ...
  const movie = await client.db("mca").collection("movies").deleteOne({ id: id });
  //   console.log(movie);

  // const val=movies.find((item)=>item.id===id)
  movie.deletedCount>0 ? response.send({msg:"Movie was deleted successfully:)"}) : response.status(404).send({msg:"Movie Not found:("});
});

app.put("/movies/:id",async function(req,res){
  const {id}=req.params;
  const data=req.body
  const result=await client.db("mca").collection("movies").updateOne({id:id},{$set:data});
